...
select 	st_astext(st_transform(GEOM,4326)), id,pais,now() as FECHAMOD,''1'' from capas_gral.pais WHERE pais in (select DISTINCT(PAIS) from capas_gral.partidos where PROVINCIA = ''' || tabla || ''');
...
SELECT IDSOFLEX_PROV::INT,B.ID,PROVINCIA,FECHAMOD,st_Astext(ST_TRANSFORM(A.GEOM,4326)),1 FROM capas_gral.provincias A INNER JOIN CAPAS_GRAL.PAIS B ON A.PAIS = B.PAIS
WHERE a.pais in (select DISTINCT(PAIS) from capas_gral.partidos where PROVINCIA = ''' || tabla || ''') ORDER BY IDSOFLEX_PROV;
...
TRUNCATE TABLE _MYSQL.DEPARTAMENTO;
 ...
 insert into _mysql.departamento  (DEPAR_PROV_ID,  depar_id,  depar_descripcion,  depar_fecha_actualizacion,  depar_habilitado,  depar_archivo_dbf,  wkt)   SELECT b.id,IDSOFLEX_PROV::INT,PROVINCIA,FECHAMOD,1,1,st_Astext(ST_TRANSFORM(A.GEOM,4326)) FROM capas_gral.partidos  a inner join capas_gral.pais b on a.pais = b.pais    where provincia = ''' || tabla || ''' and a.pais in (select DISTINCT(PAIS) from capas_gral.partidos where PROVINCIA = ''' || tabla || ''')  ORDER BY IDSOFLEX_PROV;
...
SELECT CONCAT(IDSOFLEX_PROV,idsoflex_DPTO)::INT,CONCAT(IDSOFLEX_PROV,idsoflex_local)::INT,localidad,fechamod,''1'',now()::timestamp::text,st_astext(st_transform(geom,4326)),''0'' FROM CAPAS_gRAL.LOCALIDADES WHERE PROVINCIA = ''' || tabla || ''' and localidad not like ''%ZONA RURAL%'' and idsoflex_local is not null
...
select id,arch_id,concat(shp,''.dbf'') as arch_nombre,now()::timestamp::text as arch_fecha_modificacion ,now()::timestamp::text as arch_fecha_actualizacion, st_Astext(st_transform(geom,4326))  from capas_gral.partidos where provincia = ''' || tabla || ''';
...
ALTER FUNCTION _mysql.importador_geometrias(character varying)
    OWNER TO nahuel;
