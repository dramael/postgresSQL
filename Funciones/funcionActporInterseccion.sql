...
EXECUTE (
'
create table test.act1 as select a.id,  a.fromleft, a.toleft,a.fromright, a.toright,a.localidad,  ARRAY_AGG(distinct(b.calle) order by b.calle) AS INTER 
...
select a.id, a.calle, a.fromleft, a.toleft,a.fromright, a.toright,a.localidad,  ARRAY_AGG(distinct(b.calle) order by b.calle) AS INTER 
from _cartografia.'||tabla||' a
inner join _cartografia.'||tabla||' b on st_intersects (a.geom, b.geom)
where a.id <> b.id and a.calle <> b.calle and a.calle is not null and b.calle is not null
and (a.fromleft+a.toleft+a.fromright+a.toright) =0 
...
select distinct (a.id), b.fromleft, b.toleft, b.fromright, b.toright, a.localidad from test.act2 a
...
update _cartografia.'||tabla||' a set 	fromleft = b.fromleft,
											...
											toright = b.toright,
											"check" = ''P''
from (select id, fromleft, toleft,fromright, toright,localidad from test.act3 where id in (select id from test.act3
group by id having count (id) = 1 order by 1)
order by 1) b
where a.id = b.id and a.tcalle is null and a.localidad = b.localidad and a.check is null;

delete from test.act3 where id in (select id from test.act3
group by id having count (id) = 1 order by 1);
...
update _cartografia.'||tabla||' a set 	fromleft = b.fromleft,
											...
											toright = b.toright,
											"check" = ''P''
from (select id, min(fromleft)fromleft, max(toleft)toleft, min (fromright)fromright, max(toright)toright, localidad from test.act3
group by id,localidad order by id) b
where a.id = b.id and a.tcalle is null and a.localidad = b.localidad and a.check is null;
